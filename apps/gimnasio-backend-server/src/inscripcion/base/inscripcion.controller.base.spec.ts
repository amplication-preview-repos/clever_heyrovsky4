...
const service = {
  ...
  inscripcions: () => FIND_MANY_RESULT,
  inscripcion: ({ where }: { where: { id: string } }) => {
    switch (where.id) {
      case existingId:
...
describe("Inscripcion", () => {
  ...
  test("GET /inscripcions/:id non existing", async () => {
    await request(app.getHttpServer())
      .get(`${"/inscripcions"}/${nonExistingId}`)
  ...
  test("GET /inscripcions/:id existing", async () => {
    await request(app.getHttpServer())
      .get(`${"/inscripcions"}/${existingId}`)
      .expect(HttpStatus.OK)
      .expect({
        ...FIND_ONE_RESULT,
  ...
  test("POST /inscripcions existing resource", async () => {
